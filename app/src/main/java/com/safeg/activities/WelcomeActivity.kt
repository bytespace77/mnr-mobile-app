...
class WelcomeActivity : AppCompatActivity(), View.OnClickListener {
        ...
        Thread {
            ...
            runOnUiThread {
                AndroidNetworking.get(Constants.getModuleConfig)
                    ...
                    .build()
                    .getAsObject(GetConfigResponseItem::class.java,
    ...
    override fun onClick(view: View?) {
        when (view?.id) {
            R.id.rlMalaysianPr -> {
                // ✅ Exact same as working APK — only sets isForeigner
    ...
    private fun requestPermissions(showToast: Boolean) {
        if (checkSelfPermission(android.Manifest.permission.CAMERA) == android.content.pm.PackageManager.PERMISSION_GRANTED) {
            ...
            if (showToast) Common.showToast(this, "Please allow permissions to continue.")
            requestPermissions(arrayOf(android.Manifest.permission.CAMERA), PERMISSIONS_REQUEST)
        }
    }
    ...
    // ✅ Exact same as working APK — resets request on every onStart
